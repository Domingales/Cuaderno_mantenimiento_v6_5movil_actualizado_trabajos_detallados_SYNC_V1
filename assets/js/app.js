...
<body>
  ...
  function makeTrabajoCompletadoRow(data){
    ...
    const rowTop = UI.el("div",{class:"trabajo-completado-top"});
    const ta = UI.el("input",{
      class:"input",
      type:"text",
    ...
    const btns = UI.el("div",{class:"trabajo-completado-botones"});
    const add = UI.el("button",{type:"button", class:"btn btnSmall"}, "＋");
    const del = UI.el("button",{type:"button", class:"btn btnSmall btnDanger"}, "－");
    btns.appendChild(add); btns.appendChild(del);
  ...
  function renderInfo(container){
    ...
    c.appendChild(UI.el("hr",{class:"sep"}));
    const syncInfo = UI.el("div",{class:"tiny muted", style:"margin-bottom:8px"}, "Sincronización: disponible desde Ajustes.");
    if(window.FirebaseSyncMT && typeof FirebaseSyncMT.getStatus === "function"){
      const st = FirebaseSyncMT.getStatus();
      syncInfo.textContent = "Sincronización: " + (st.text || "estado no disponible");
    }
    c.appendChild(syncInfo);
    c.appendChild(btn("⏳ SINCRONIZAR", "btnSuccess", ()=>{
      if(window.FirebaseSyncMT && typeof FirebaseSyncMT.openModal === "function") FirebaseSyncMT.openModal();
      else UI.toast("Sincronización no disponible.");
    }));
