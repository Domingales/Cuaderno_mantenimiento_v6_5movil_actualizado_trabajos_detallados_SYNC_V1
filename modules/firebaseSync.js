...
(function(){
  ...
  function ensureMeta(r, action){
    ...
    const act = action || "guardado";
    const mustRefresh = /importacion|restauracion|excel|json/i.test(String(act));
    const t = mustRefresh ? now() : (r.updatedAt || r.modificadoEn || now());
    r.updatedAt = t; r.modificadoEn = t;
    r.syncDeviceId = getDeviceId();
    r.syncAction = act;
    return r;
  }
  ...
  function renderWidget(){
    const w=document.getElementById("syncWidget");
    if(w) w.remove();
  }
